    ...
    private void FixedUpdate()
    {
        if (!PuckScript.WasGoal) {
            ...
            if (rbPuck.position.y < puckBoundary.Down)
            {
                ...
                targetPosition = new Vector2(Mathf.Clamp(rbPuck.position.x + offsetXFromTarget, playerBoundary.Left, playerBoundary.Right), startingPosition.y);
            }
            else
            {
                ...
                targetPosition = new Vector2(Mathf.Clamp(rbPuck.position.x, playerBoundary.Left, playerBoundary.Right), Mathf.Clamp(rbPuck.position.y, playerBoundary.Down, playerBoundary.Up));
            }
