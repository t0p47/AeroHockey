...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            //Vector3 preTouchWorldPos = Camera.main.ScreenToWorldPoint(Input.GetTouch(i).position);
            Vector2 touchWorldPos = Camera.main.ScreenToWorldPoint(Input.GetTouch(i).position);
            Debug.Log("touchWorldPos: "+touchWorldPos+", touchPos: "+Input.GetTouch(i).position);
